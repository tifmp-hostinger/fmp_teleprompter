...
export function createScript({ title = '', body = '' } = {}) {
  const now = Date.now();
  const script = { id: uid(), title, body, createdAt: now, updatedAt: now, wpm: null, fontSize: null };
...
export function importBackup(json) {
  ...
  for (const s of data.scripts) {
    ...
    byId.set(id, { ...s, id, updatedAt: s.updatedAt || Date.now(), createdAt: s.createdAt || Date.now() });
    count++;
  }
